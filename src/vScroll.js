function on(el, evtName, handler) {
  el.addEventListener(evtName, handler, false)
}
function off(el, evtName, handler) {
...
vScroll.prototype.init = function() {
  ...
  var sh = parseInt(window.getComputedStyle(this.scroller, null)["height"])||this.scroller.offsetHeight, wh = parseInt(window.getComputedStyle(this.wrapper, null)["height"])||this.wrapper.offsetHeight;
  this.max = wh - sh;
  this.speed = 0.8;
  this.start = null;
  this.dir = 1;
...
vScroll.prototype.enable = function() {
  ...
  on(document.body, "touchmove", function(evt) {
    ...
    if((self.pos.y > 0 && deltaY > 0) || (self.pos.y < self.max && deltaY < 0)) {
      ...
      deltaY>0? (deltaY > 1 ? null : (deltaY = 1)) : (deltaY < -10 ? (deltaY = -1) : null);
      self.pos.y += deltaY;
      self.speed *= .8
    }
    self.scroller.style.transform = "translateY("+self.pos.y+"px) translateZ(0px)";
  });
  on(document.body, "touchend", function(evt) {
    if(!self.isEnabled) return false;
    evt.preventDefault();
    if(self.pos.y > 0) {
